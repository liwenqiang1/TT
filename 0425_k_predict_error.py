...
import sys
#from AE.test_error import error_calculate
...
for i in range(len(C2_deltaV)):
    ...
    if C2_deltaV[i]/dV <= 1.8:
        ...
        c2 = 1.8-0.1*random.random()
        #print(c2)
        c2_list.append(c2)
        c1 = c2+1.2+0.1*random.random()
        #print(c1)
        c1_list.append(c1)
...
#输出
result = pd.DataFrame(columns=['new_RJY1', 'new_RJY2', 'new_gfjf'])
result['created_at'] = QuarterData['created_at'][:-1]
result['new_RJY1'] = c1_list
result['new_RJY2'] = c2_list
result['new_gfjf'] = gfjf_list
result['RJY1'] = np.delete(RJY1, [-1])
result['RJY2'] = np.delete(RJY2, [-1])
result['GFJF'] = np.delete(GFJF, [-1])
order = ['created_at', 'GFJF', 'new_gfjf', 'RJY1', 'RJY2', 'new_RJY1', 'new_RJY2']
result = result[order]
